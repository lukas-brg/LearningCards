...
class LearningCard(ABC):
    ...
    def validate_format(self):
        if self.back.strip() == "":
            raise CardSyntaxError(
                f"Empty backside on card {self.id+1}: \n" + self.front)
    ...
    def to_html(self) -> HtmlNode:
        ...
        front_div = HtmlNode("div", *front_elems,
                             set_class=f"front {card_type_name}")
        ...
        # Whole backside, including collapse button etc.
        self.back_div = back_div
        self.answer_div = answer  # Only user written text

        card_div = HtmlNode("div", front_div, back_div,
                            set_class=f"card {card_type_name}", id=self.id)
...
class AnswerCard(QuestionCard):
    ...
    def parse_backside(self, back_str: str) -> list[HtmlNode]:
        ...
        pre_answer_html = parse_markdown(
            pre_answer, paragraph=pre_answer[-1] == '\n', add_linebreak=False) if pre_answer else []
        post_answer_html = parse_markdown(
            post_answer, paragraph=post_answer[0] == '\n', add_linebreak=False) if post_answer else []
        input = SelfClosingTag("input", type="text", set_class="answer-input",
                               name=f"answer_field{self.id}", autocomplete="off")
        answer_span = HtmlNode(
            "span", answer, name=f"answer{self.id}", set_class="answer-span")
        ...
        form = HtmlNode("form", input, SelfClosingTag(
            "br"), btn, action=f"javascript:void(0);")

        answer_div = HtmlNode(
            "div", set_class="answer-content", name=f"answer_content{self.id}")
        answer_div.add_children(
            HtmlNode("p", *pre_answer_html, answer_span, *post_answer_html))
...
class MultipleChoiceCard(QuestionCard):
    ...
    def is_multi_correct(string): return string.startswith(
        MultipleChoiceCard.multi_correct)

    def is_multi_incorrect(string): return string.startswith(
        MultipleChoiceCard.mult_incorrect)

    def is_multi(string): return MultipleChoiceCard.is_multi_correct(
        string) or MultipleChoiceCard.is_multi_incorrect(string)

    def parse_multi(self, lines: list[str], start: int):

        multi_div = HtmlNode("form", set_class="multi",
                             name=f"multiform{self.id}", action=f"javascript:void(0)")
        ...
        post_elems = parse_markdown(
            "".join(lines[i:])) if i < len(lines) - 1 else ""
