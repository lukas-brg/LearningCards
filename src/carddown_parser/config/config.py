...
USR_CONFIG_PATH = os.path.join(
    appdirs.user_config_dir("carddown"), CONFIG_FILE_NAME)
    ...
    def overwrite(self, cfg_dict: dict[str, any]):
        for key, val in cfg_dict.items():
            if self._check_key(key):
                ...
                print(
                    f"Warning: '{self._key}.{key}' is not a valid config attribute.")
...
class Config:
    ...
    def _get_attribute_dict(objs: list):
        cfg_dict = {
            obj._key:
                {attr: getattr(obj, attr) for attr in dir(obj) if not callable(
    ...
    def config_set(arg: str):
        ...
        for ukey, uval in usr_cfg.items():
            ...
            if not found:
                print(
                    f"Warning: '{ukey}={uval}' is not a valid config attribute. Disregarding...")
...
def load_configs(args):
    ...
    if args.toc_lvl is not None:
        if config.document.table_of_contents:
            ...
            print(
                "Warning. Table of contents is disabled, so the '--toc-lvl' parameter is disregarded.")
    ...
    if args.align:
        if args.align == "center" or args.align == "left":
            ...
            print(
                f"Warning: Invalid aligh option '{args.align}'. Disregarding...")
    ...
    if args.lang is not None:
        if args.lang not in LOCALS:
            print(
                f"Warning: Invalid lang option '{args.lang}'. Only {list(LOCALS.keys())} are supported.")
    ...
    if args.margin is not None:
        ...
        if margin < 0 or margin >= 100:
            print(
                f"Warning: margin parameter expects a positive number in range [0...100.] (margin={args.margin} given). Disregarding...")
        else:
