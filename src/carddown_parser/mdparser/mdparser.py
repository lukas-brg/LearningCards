...
def is_ul(line): return line.strip().startswith(
    tuple((list_token + ' ' for list_token in config.mdparser.list_item_chars)))


def is_ol(line): return re.search(ORDERED_LIST_PATTERN, line.strip())
def is_dd(line): return re.search(DEF_PATTERN, line)
def is_hr(line): return re.match(r"[*]{3,}|[-]{3,}", line.replace(" ", ""))


def is_list(line): return is_ul(line) or is_ol(line)
def is_table(line): return re.search(TABLE_PATTERN, line)
def is_latex(line): return line.startswith("$$")
def is_heading(line): return re.search(HEADING_PATTERN, line)


def is_checked(line): return line.startswith(CHECKED_PATTERN)
def is_footnote(line): return re.search(FOOTNOTE_PATTERN, line)
def is_unchecked(line): return line.startswith(UNCHECKED_PATTERN)
def is_task_list(line): return is_checked(line) or is_unchecked(line)
def is_block_quote(line): return line.lstrip().startswith(">")


def is_codeblock_fenced(line): return re.match(r"^```\s*(\S*)$", line.rstrip())


def is_codeblock_indented(line): return leading_whitespaces(
    line) >= 4 or line.strip() == "" or line.startswith("\t")
...
def get_token_types() -> list[InlineToken]:
    ...
    global token_types
    if not token_types:
        token_types = [TokenType for TokenType in InlineToken.get_all_token_types(
        ) if TokenType.__name__ not in config.mdparser.ignore_inline_tokens]
    return token_types
...
def parse_link(line: str, start: int) -> HtmlNode:
    ...
    link_contentless = "[" + line[content_end:link_end]
    ...
    while i < end:
        ...
        if i in tokens and parse_content:
            ...
            if isinstance(token, LinkToken):
                ...
                _parse_inline(line, tokens, html_elem, token.content_start,
                              token.content_end, token.parse_content)
                i = token.end
...
def parse_inline(line: str) -> list[HtmlNode]:
    ...
    if not tokens:
        ...
        _parse_inline(line, tokens=tokens, parent=temp_container,
...
def make_codeblock_elem(code_lines: list[str], lang: str):
    ...
    code_container = HtmlNode("div", HtmlNode(
        "pre", code), set_class="code-block-container")
    scrollbar_container = HtmlNode(
        "div", code_container, set_class="code-block-scrollbar-container")
...
def parse_codeblock_fenced(lines: list[str], start: int) -> tuple[HtmlNode, int]:
    ...
    if end == len(lines):
        show_warning_msg(
            f"Unclosed multiline code detected. (line {start+1}-{end})")
...
def parse_codeblock_indented(lines: list[str], start: int) -> tuple[HtmlNode, int]:
    def remove_indentation(line): return re.sub(r"^(\t|\s{4})", "", line)

    end = find_line(
        lines, start+1, function=is_codeblock_indented, negate=True)
    code_lines = [remove_indentation(line)
                  for line in multiline_strip(lines[start:end])]
    ...
    for i, line in enumerate(lines[start+2:], start+2):
        ...
        if len(row_matches) != num_cols:
            raise MarkdownSyntaxError(
                f"Inconsistent number of columns in table.\n'{line}'")
...
def parse_blockquote(lines: list[str], start: int, depth=1) -> tuple[HtmlNode, int]:
    ...
    if text.strip():
        blockquote.add_children(
            HtmlNode("p", *parse_markdown(text, paragraph=False)))
...
def parse_blockrule(parse_func: Callable, start: int, lines: list[str], **kwargs) -> tuple[HtmlNode, int]:
    ...
    try:
        ...
        if ENABLE_DEBUG:
            traceback.print_exc()
        print(
            f"Warning: Failed to parse blockrule '{parse_func.__name__.replace('parse_', '')}'. {type(e).__name__}: {e}")
...
def parse_latex(lines: list[str], start: int) -> HtmlNode:
    ...
    if config.document.prerender_latex:
        ...
        node = HtmlNode("div", "$$" + latex_str + "$$",
                        set_class="latex block-latex")
...
def parse_markdown(markdown: list[str] | str, paragraph=True, add_linebreak=True) -> list[HtmlNode]:
    ...
    while i < len(lines):
        ...
        if re.match(r"^<.+>$", line.strip()):
            ...
            latex, i = parse_blockrule(
                parse_func=parse_latex, lines=lines, start=i)
            p = append_paragraph(parsed_elems, p, paragraph)
            parsed_elems.append(latex)

        elif is_codeblock_indented(line):
            code, i = parse_blockrule(
                parse_func=parse_codeblock_indented, lines=lines, start=i)
            p = append_paragraph(parsed_elems, p, paragraph)
            parsed_elems.append(code)

        elif is_codeblock_fenced(line):
            code, i = parse_blockrule(
                parse_func=parse_codeblock_fenced, lines=lines, start=i)
            p = append_paragraph(parsed_elems, p, paragraph)
            parsed_elems.append(code)
            ...
            list, i = parse_blockrule(
                parse_func=parse_list, lines=lines, start=i)
            p = append_paragraph(parsed_elems, p, paragraph)
            parsed_elems.append(list)

        elif is_table(line):
            table, i = parse_blockrule(
                parse_func=parse_table, lines=lines, start=i)
            p = append_paragraph(parsed_elems, p, paragraph)
            ...
            quote, i = parse_blockrule(
                parse_func=parse_blockquote, lines=lines, start=i, depth=get_blockquote_depth(line))
            div = HtmlNode("div", quote, set_class="blockquote-div")
            p = append_paragraph(parsed_elems, p, paragraph)
            parsed_elems.append(div)

        elif is_footnote(line):
            footnote_div, i = parse_blockrule(
                parse_func=parse_footnotes, lines=lines, start=i)
            p = append_paragraph(parsed_elems, p, paragraph)
